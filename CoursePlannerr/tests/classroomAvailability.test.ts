...
function createCourse(
  ...
  return {
    id,
    universityId,
    universityName: universityId.toUpperCase(),
    crn: id,
    code,
    title: code,
    instructor: 'TBA',
...
test('ignores placeholder classroom locations', () => {
  ...
  assert.equal(parseClassroomLocation('Online Section 01'), null);
});
...
test('groups meetings by room and computes room free slots', () => {
  ...
  const room101 = availability.find((room) => room.roomKey === 'main campus::nicely hall::101');
  assert.ok(room101);
...
test('keeps identical building-room names separate across campuses', () => {
  const availability = buildClassroomAvailability([
    createCourse('1', 'CMPS 271', {
      days: ['M'],
      start: '09:00',
      end: '10:15',
      location: 'Nicol Hall 101',
      type: 'Lecture',
    }, 'Beirut Campus', 'lau'),
    createCourse('2', 'CMPS 272', {
      days: ['M'],
  ...
  assert.deepStrictEqual(
    availability.map((room) => room.campus),
